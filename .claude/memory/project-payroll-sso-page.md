...
**rev3 (เย็น 14ก.ค. — โอส่งไฟล์จริงหมิว "2569 เงินสมทบประกันสังคม รวม.xlsx" ใน Downloads):**
- โครงไฟล์หมิว: แท็บ ม.ค.–ธ.ค. คอลัมน์ เลขบัตร/คำนำหน้า/ชื่อ/สกุล/เงินจ่ายเต็ม/โบนัส/
  ค่าจ้าง(ฐาน)/รายได้อื่นๆ/เงินสมทบ/0.05/ประเภท + แท็บ "บันทึกประเภทพนักงาน" = ทะเบียน
  488 คน (A=เลขบัตร B=คำนำหน้า C=ชื่อ D=สกุล **I=ประเภท**) + แท็บสรุปปี; **เงินจ่ายเต็ม
  ของหมิว = หลังหักน้ำมันเหมา** (มีเดือนติดลบ เช่น ธัชชนพล เม.ย. −14,157)
- export `/payroll/sso/export.xlsx` คอลัมน์ตรงเป๊ะ วางลงแท็บเดือนได้เลย; รวมคนข้ามไซท์
  เป็นแถวเดียว; ฐาน = สปส.จริง÷อัตรา (การันตี ฐาน×อัตรา=ยอดนำส่ง); คนไม่ถูกหักแยกท้ายไฟล์
- ทะเบียนหมิว → `app/sso_meta_data.py` (PII — ห้ามย้ายไป static/); อัปเดต:
  `python tools/build_sso_meta.py "<ไฟล์หมิว>"` แล้ว deploy (เป็น .py เพราะ
  deploy_mvp.sh ก็อปเฉพาะ app/*.py — [[reference-deploy-mvp-selfverify]])
