...
**ภาคดึก (เพิ่ม):** coverage เส้นทางเงิน (services 55%, payroll 70%) → **ยึดกติกาเงินเป็นเทสต์ 49 ตัว** (ผ่านรันแรกทุกตัว = engine ตรงกติกาโอ; ชุด3 = lcb_mao KB share + lcb_mixed สมการเต็ม → โหมดเงินทุกโหมด 3 ไซท์มีเทสต์ engine ครบ): ชุด1 (29) กติการอบจ่าย 3 ไซท์ (เดิม 0 เทสต์! + property test ปีเต็ม) + โหมดจ่าย AYU 4 โหมด (กันบั๊ก revenue×60% กลับมา); ชุด2 (18) ภาษีก้าวหน้าครบ 8 ขั้น + driver_calc_price(KB) + จำแนกวัน LCB ผสม 0.60±0.05/<0.15/กลาง + รถจอดคั่นช่วงเหมา (tie→เหมา) + BigC เรทน้ำมัน 16/32.15 (ไม่มีงบ=ไม่คิด); โมดูล PDF 0% = ต้องใช้ Chrome (ยอมรับ); fix POD ตู้คู่ "AAAA/BBBB"; restore_drill.py ซ้อมกู้คำสั่งเดียว (รันจริงผ่าน); /admin/plan โชว์คิวรอโอ (WAITING_ON_OAT.md — อัปเดตไฟล์นี้เมื่อคิวเปลี่ยน)
