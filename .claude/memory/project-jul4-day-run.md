...
**ภาคดึก (เพิ่ม):** coverage เส้นทางเงิน (services 55%, payroll 70%) → **ยึดกติกาเงินเป็นเทสต์ 29 ตัว**: กติการอบจ่าย 3 ไซท์ (เดิม 0 เทสต์! รวม property test ปีเต็ม) + โหมดจ่าย AYU 4 โหมด (กันบั๊กเก่า revenue×60% กลับมา); โมดูล PDF 0% = ต้องใช้ Chrome (ยอมรับ); fix POD ตู้คู่ "AAAA/BBBB"; restore_drill.py ซ้อมกู้คำสั่งเดียว (รันจริงผ่าน); /admin/plan โชว์คิวรอโอ (WAITING_ON_OAT.md — อัปเดตไฟล์นี้เมื่อคิวเปลี่ยน)
