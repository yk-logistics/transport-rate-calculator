...
def test_net_column_is_the_money_truth():
    """ของจริง 10ก.ค.: บางบรรทัดชีทใส่ VAT ไว้ในช่องสุทธิแต่ไม่กรอกช่อง VAT
    (1,248.50 → สุทธิ 1,335.90) และบางบรรทัดชื่อ 'VAT' มียอดเฉพาะช่องสุทธิ
    → ยึด "ราคาสุทธิ" เป็นความจริงเรื่องเงิน แล้วให้ vat ปิดส่วนต่าง"""
    hdr = ["วันที่", "เลขกิโลเมตร", "บริษัท", "รายละเอียด", "จำนวน", "ราคา", "รวม",
           "ส่วนลด", "ภาษีมูลค่าเพิ่ม", "ราคาสุทธิ"]
    tab = [hdr,
           ["05/01/24", "", "ร้าน ก", "เปลี่ยนถ่ายน้ำมันเครื่อง", " 1.00 ", " 1,248.50 ",
            " 1,248.50 ", "", "", " 1,335.90 "],
           ["", "", "", "VAT", "", "", "", "", "", " 34.81 "],
           ["", "", "", "อัดจาระบี", " 1.00 ", " 180.00 ", " 180.00 ", "", "", ""]]
    p = rm.parse_tab("71-6802", tab)
    a, b, c = p.bills[0].lines

    assert a["vat"] == 87.40 and round(a["total"] - a["discount"] + a["vat"], 2) == a["net"]
    assert b["vat"] == 34.81 and b["total"] == 0.0        # บรรทัด 'VAT' เป็นรายการเดี่ยว
    assert c["net"] == 0.0 and c["vat"] == 0.0 and c["total"] == 180.0   # ชีทไม่กรอกสุทธิ → นับจากรวม


def test_parse_tab_tolerates_missing_optional_columns():
    """ของจริง: บางแท็บไม่มีคอลัมน์ 'ส่วนลด'/'ภาษีมูลค่าเพิ่ม' เลย — ต้องอ่านต่อได้ (เป็น 0)."""
    hdr = ["วันที่", "เลขกิโลเมตร", "บริษัท", "รายละเอียด", "จำนวน", "ราคา", "รวม", "ราคาสุทธิ"]
    tab = [hdr,
           ["05/01/24", "", "ร้าน ก", "ผ้าเบรก", " 2.00 ", " 300.00 ", " 600.00 ", " 600.00 "]]
    p = rm.parse_tab("71-6802", tab)
    assert len(p.bills) == 1
    ln = p.bills[0].lines[0]
    assert ln["total"] == 600.0 and ln["discount"] == 0.0 and ln["vat"] == 0.0
