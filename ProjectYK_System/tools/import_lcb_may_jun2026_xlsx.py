"""
Import LCB daily for cycle 2026-05-16 – 2026-06-15 from the LOCAL Excel file
(โอ's hand-edited 'วางบิล YK VOLVO.xlsx', sheet renamed to "Daily").

Why this exists (2026-06-27): โอ edited the LOCAL .xlsx (filled missing 15/6
prices, added OT, corrected 21/5 KLND for นิพล+วิโรจน์). Those edits are NOT in
the live Google Sheet — gsheet is stale (trip≈701k vs local≈813k, rev differs
~165k). โอ confirmed: **Excel local is source of truth**. So this importer reads
the .xlsx instead of gspread, but reuses the SAME column-name mapping and the
SAME source tag 'lcb_may-jun2026' + --wipe-prior, so it cleanly REPLACES the
stale gsheet import rather than duplicating.

เงินเบิก / ค่าเสียเวลา columns intentionally NOT imported (live in สดย่อย/petty).

Run (from repo root "Project YK"):
  ProjectYK_System/app/.venv/Scripts/python.exe ProjectYK_System/tools/import_lcb_may_jun2026_xlsx.py --dry-run
  ProjectYK_System/app/.venv/Scripts/python.exe ProjectYK_System/tools/import_lcb_may_jun2026_xlsx.py --wipe-prior
"""
...
TAB_NAME = "Daily"
IMPORT_SOURCE = "lcb_may-jun2026"  # SAME tag as gspread importer → --wipe-prior replaces it
