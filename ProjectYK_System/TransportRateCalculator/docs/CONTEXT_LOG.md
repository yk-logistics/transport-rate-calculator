...
### บริบทจากผู้ใช้
- ผู้ใช้ชี้จากหน้า `เที่ยวทั้งหมด` ว่าแถวทะเบียน `71-5042` แสดงยอดในคอลัมน์ผิด และระบุว่าควรย้ายไปช่อง `เสียเวลา+100%`
- ต้องการแก้ให้ตรงจุดทันทีในหน้ารายงานที่ใช้งานอยู่

### การตัดสินใจรอบนี้
- ใช้แนวทาง hotfix เฉพาะจุด (surgical) ในไฟล์รายงาน HTML ปัจจุบัน เพื่อแก้เร็วและไม่กระทบการคำนวณส่วนอื่น
- คงยอดคอลัมน์อื่นของแถวเดิมทั้งหมด และย้ายเฉพาะ bucket surcharge จาก `+50%` ไป `+100%`

### สิ่งที่ทำแล้ว
- แก้ไฟล์ `reports/oatside-pg-2026/trips.html`:
  - แถวทะเบียน `71-5042` (`Origin In: 2026-04-21 12:28:30`) ย้ายค่า `6,546.00` จาก `เสียเวลา+50%` -> `เสียเวลา+100%`
- แก้ไฟล์ `reports/oatside-pg-2026/plates/71-5042.html` ให้ตรงกันแบบเดียวกัน
- Verify:
  - ตรวจ pattern ก่อน-หลังด้วย `rg` ยืนยันว่า pattern เดิมไม่เหลือ และ pattern ใหม่อยู่ครบทั้ง 2 ไฟล์
  - `ReadLints` ไม่พบปัญหาในไฟล์ที่แก้
...
## 2026-05-08 (Session Summary #166 - ยืนยัน policy เรท 12-15 เม.ย. = 8,000)

### บริบทจากผู้ใช้
- ผู้ใช้ยืนยันเงื่อนไขราคา Oatside ว่าช่วงวันที่ `12-15 เม.ย.` ต้องเป็นเรท `8,000` เพราะอยู่ใน Base rate `50.00-50.99`
- ใช้เพื่อยืนยันความถูกต้องของการเติมข้อมูลรายงาน/Excel ที่อ้างอิงเรทราคาในช่วงดังกล่าว

### การตัดสินใจรอบนี้
- ล็อกเป็น domain fact ว่า **12-15 เม.ย. 2026 = base trip 8,000** ภายใต้ base fuel band `50.00-50.99`
- คง policy เดิมวันที่อื่นตาม config/report rules ปัจจุบัน

### สิ่งที่ทำแล้ว
- ยืนยันกลับผู้ใช้ว่า logic ที่ใช้เติมไฟล์ตรงตามเงื่อนไขนี้
- บันทึก fact เข้าบริบทถาวร เพื่อกันการสลับไปใช้ 7,500 ในช่วงวันดังกล่าวรอบถัดไป

### Action ถัดไป
- รอบแก้ข้อมูลครั้งถัดไปให้ยึดวัน `12-15 เม.ย.` เป็นจุดตรวจภาคบังคับ (sanity check) ว่า base trip ยังเป็น `8,000`

---

## 2026-05-08 (Session Summary #167 - แก้ค่าเสียเวลาให้คิดจากค่าขนส่ง 50%/100% แล้ว push)

### บริบทจากผู้ใช้
- ผู้ใช้แจ้งว่าคอลัมน์ค่าเสียเวลาในรายงาน Oatside ยังไม่คิดจากค่าขนส่งจริง เช่น +50% แล้วขอให้แก้ทันทีและ push
- งานต้องทำแบบ end-to-end โดยไม่หยุดที่อธิบายปัญหา

### การตัดสินใจรอบนี้
- ใช้แนวทาง hotfix ที่ไฟล์รายงาน HTML ที่ publish แล้ว (`reports/oatside-pg-2026`) เพื่อแก้ผลลัพธ์ที่ผู้ใช้เห็นทันที
- เพิ่ม guardrail ฝั่งหน้าเว็บให้ re-calc เฉพาะแถวที่มีค่าเสียเวลาอยู่แล้ว โดยอ้าง `ค่าขนส่ง(฿)` ในแถวเดียวกัน แล้วคง format 2 ตำแหน่ง

### สิ่งที่ทำแล้ว
- แก้ไฟล์ `reports/oatside-pg-2026/trips.html` และ `reports/oatside-pg-2026/plates/*.html` รวม 14 ไฟล์:
  - เติม script `parseMoneyText()` + `tuneTable()` เพื่อคำนวณใหม่:
    - `เสียเวลา+50%(฿) = ค่าขนส่ง(฿) x 0.5`
    - `เสียเวลา+100%(฿) = ค่าขนส่ง(฿) x 1.0`
  - เติม `tuneDailyBadges()` ให้ badge ข้อความค่าเสียเวลาในตารางรายวันแสดงยอดใหม่ที่ผูกค่าขนส่งจริง
- ตรวจยืนยันว่าไฟล์เป้าหมายทั้งหมดมีสคริปต์ hotfix ครบด้วยการค้นหา signature (`parseMoneyText`) แล้ว
...
## 2026-05-20 (Session Summary #188 - LCB fuel Pages 2 trucks fix)

### บริบทจากผู้ใช้
- เปิด Pages เห็นแค่ 9628/0419 (2/16) — สงสัยว่าตารางกรองเฉพาะต้องเติม; สับสน URL หน้าแรก TR calculator กับ fuel dispatch

### การตัดสินใจรอบนี้
- โค้ดตารางแสดง **ทุก assignment จาก parse แผน** อยู่แล้ว; กล่องส้ม “ยังต้องเติม” แยกด้านล่าง
- Pages ค้างเพราะ build จาก `lcb_plan_sample.txt` (2 คัน KAO ทดสอบ) ไม่ได้ push แผน `21.05.26.txt`
- เพิ่มหมายเหตุใน HTML: ลิงก์สาธารณะ `…/reports/lcb-fuel-dispatch/` ≠ `…/transport-rate-calculator/`

### สิ่งที่ทำแล้ว
- rebuild จาก `Downloads/21.05.26.txt` + GPS CSV → 16 แถว; ยังต้องเติม buffer: 9628, 0419
- push commit `79fa6c8` → origin main

### Action ถัดไป
- โอเปิดลิงก์ fuel dispatch หลัง hard refresh; แผนวันใหม่ = รัน bat แล้ว push อีกครั้ง

---

## 2026-05-20 (Session Summary #189 - LCB GPS inbox + budget UI)

### บริบทจากผู้ใช้
- หน้าแผนไม่ขึ้น GPS (2 คัน / คอลัมน์ `--`); ต้องการสั่งเติม 71-6804 +30 ล.; ปรับวงเงินได้; ไฟล์ใน `reports/gps_inbox/`

### การตัดสินใจรอบนี้
- สาเหตุ: build จากแผนตัวอย่าง 2 คัน ไม่ใช่ `21.05.26.txt`; xlsx Wialon ต้องสืบทะเบียนจากแถวย่อย 7.1
- bat หยิด `gps_inbox` ก่อน Downloads; ห้าม fallback CSV เงียบ; `--add-fuel 71-6804=30`
- งบต่ำ/สูง แก้บนหน้าได้ (`--budget-low/high` + input toolbar)

### สิ่งที่ทำแล้ว
- rebuild จาก `gps_inbox/21.05.26.txt` + xlsx 19:06 → 15 คัน + GPS (8684=133@19:05, 6804 GPS67+เติม30=97)
- แก้ `load_fuel_xlsx`, bat, budget inputs ใน HTML

### Action ถัดไป
- โอเปิด HTML ใหม่ Ctrl+F5; ปรับงบบน toolbar; push Pages ถ้าต้องการเว็บสาธารณะ

---

## 2026-05-21 (Session Summary #190 - TR Calculator ปัดเศษค่าขนส่ง)

### บริบทจากผู้ใช้
- ต้องการกำหนดวิธีปัดเศษค่าขนส่งได้: หลักสิบ, หลักหน่วย, หน่วย 5/0, หรือปัดทศนิยม

### การตัดสินใจรอบนี้
- เพิ่มตัวเลือกใน Step 4 (`priceRoundingMode`) — จำค่าใน `localStorage` (`yk_price_rounding_v1`)
- แถวฐาน (Base Rate) ใช้ตัวเลขที่ผู้ใช้กรอกตรง ๆ ไม่บังคับปัด; แถวอื่นปัดตามโหมดแล้วคำนวณ % และผลต่างจากฐานใหม่
- ราคาเป้าในส่วนต้นทุน + ปุ่ม “ใช้ราคาเป้า” ใช้กฎปัดเศษเดียวกัน

### สิ่งที่ทำแล้ว
- อัปเดต `transport_rate_calculator.html` + sync `index.html` (GitHub Pages)
- สรุปการปัดในการ์ดผลลัพธ์และ Export Excel

### Action ถัดไป
- โอทด Step 4 → สร้างตาราง แต่ละโหมดปัด; push Pages ถ้าต้องการเว็บสาธารณะอัปเดต

---

## 2026-05-21 (Session Summary #191 - TR Calculator ลูกค้า/หลายโรงงานบนหน้าผลลัพธ์)

### บริบทจากผู้ใช้
- หน้าสุดท้ายต้องใส่ชื่อลูกค้า + โรงงานหลายแห่ง + ราคาเสนอ (เช่น Tianhai 4800, Geelong 5600 จาก LINE)
- พิมพ์/ส่งออกเดิมไม่บอกลูกค้า/โรงงาน; ต้องการปุ่มเพิ่มแถว (default 1 แถว)

### การตัดสินใจรอบนี้
- กล่อง「ข้อมูลเสนอราคา」บนหน้าผลลัพธ์: ลูกค้า 1 ช่อง + แถวโรงงาน/ราคา/หมายเหตุ (เริ่ม 1 แถว, + เพิ่มได้)
- กล่องสรุปด้านบน (`quotePrintBlock`) แสดงใน PDF/PNG/Excel; ฟอร์มแก้ไขซ่อนตอน capture
- ปุ่ม「ราคาปัจจุบัน」ดึงจากแถวไฮไลต์「ราคาปัจจุบัน」ในตารางน้ำมัน

### สิ่งที่ทำแล้ว
- อัปเดต `transport_rate_calculator.html` + sync `index.html`

### Action ถัดไป
- โอลองกรอกตามรูป LINE 6 โรงงาน → Export PDF ตรวจหัวกระดาษ

---

## 2026-05-21 (Session Summary #192 - TR Calculator โหมด 2 โรงงานละตาราง)

### บริบทจากผู้ใช้
- เลือกโหมด 2: หลายโรงงาน แต่ละโรงมีตารางค่าขนส่งชุดตัวเอง (Base/ช่วงฐานต่างกันได้)

### การตัดสินใจรอบนี้
- ช่วงน้ำมัน + วิธีปรับ (%) จากวิซาร์ดใช้ร่วมทุกโรง; โรงละ **Base Rate**, **ช่วงฐาน**, **% ปรับเฉพาะโรง** (ถ้าติ๊ก), ราคาเสนอ, หมายเหตุ
- หน้าผลลัพธ์: สรุปลูกค้า + ตารางสรุปรายโรง + ตารางน้ำมันซ้อนกันโรงละบล็อก; PDF/Excel รวมทุกโรง

### สิ่งที่ทำแล้ว
- Refactor `computeResultData` + `factoryBlocks`; ปุ่ม + เพิ่มโรงงาน, คัดลอก Base จากโรง 1, ราคาปัจจุบันจากตารางโรงนั้น

### Action ถัดไป
- โอทด 6 โรงจาก LINE (Base ต่างกัน) → Export PDF

---

## 2026-05-21 (Session Summary #191 - LCB แผน 22.05.26 ชื่อลูกค้า WHALE/เหรินเหอ)

### บริบทจากผู้ใช้
- แผน `22.05.26.txt` มีบล็อก WHALE5 [เหรินเหอ5] แต่หน้าแผนน้ำมันโชว์แค่คลังวาฬ+Oatside; คันเหรินเหอขึ้น Unknown ในรายการเติม

### การตัดสินใจรอบนี้
- เพิ่มแมป WHALE/เหรินเหอ/RERNHER → job `เหรินเหอ` (50 ล./เที่ยว ตู้ 40')
- ชื่อโชว์จากหัวข้อ `[...]` ในแผน LINE (`customer_label` / `job_display`)
- HTML แสดงทุกคันที่ parse ได้ (ไม่กรองเฉพาะ job เดิม); คลังวาฬนับ `Bol.` + `Con.` (รวมฟรีโซน 71-6803)

### สิ่งที่ทำแล้ว
- แก้ `parse_lcb_plan_txt.py`, `build_lcb_fuel_dispatch_from_plan.py`; rebuild แผน 22.05.26 → 9 คัน (8+Oatside)

### Action ถัดไป
- โอ Ctrl+F5 เปิด `lcb_fuel_dispatch_plan.html`; ลูกค้าใหม่เพิ่มในแผน → ใส่ pattern ใน `SECTION_JOB_HINTS` หรือใช้หัวข้อ `[ชื่อลูกค้า]`

---

## 2026-05-21 (Session Summary #192 - NHL แยกคลังวาฬ 4 ตู้ / ฟรีโซน 1 ตู้)

### บริบทจากผู้ใช้
- แผน `NHL3 [วาฬ4ตู้2+ฟรีโซน1]` = 5 ตู้รวม: วาฬ 4 ตู้ 2 คัน, ฟรีโซน 1 ตู้ 1 คัน

### การตัดสินใจรอบนี้
- `Bol. 53-449 Agent.` นำหน้าบล็อกฟรีโซน — **ไม่**นับเป็นตู้วาฬของคันก่อนหน้า (เคยทำให้ 0419 ได้ 3 เที่ยว)
- หัวหลังบรรทัด `เปิดคลังฟรีโซน` → คันนั้นเป็น job `ฟรีโซน` นับ `Con.[20]` เท่านั้น
- วาฬนับเฉพาะ `Bol.` ที่ไป `คลังปลาวาฬ`

### สิ่งที่ทำแล้ว
- แก้ `parse_lcb_plan_txt.py`; rebuild 22.05.26: 0419/0421 คลังวาฬ 2 เที่ยว, 6803 ฟรีโซน 1 เที่ยว; HTML บรรทัดสรุป NHL

### Action ถัดไป
- โอ Ctrl+F5 หน้าแผน; ยืนยันสูตร 25 ล./เที่ยว ฟรีโซนตรงงานจริงหรือไม่

---

## 2026-05-21 (Session Summary #193 - เหรินเหอ 70 ล./เที่ยว)

### บริบทจากผู้ใช้
- งานเหรินเหอ (โรงงานเหรินเหอ / WHALE5) ใช้น้ำมันประมาณ **70 ล./เที่ยว** ไม่ใช่ 50

### สิ่งที่ทำแล้ว
- ล็อก `L_PER_TRIP["เหรินเหอ"] = 70`; rebuild แผน 22.05.26

### Action ถัดไป
- โอ Ctrl+F5 หน้าแผน — ดูคอลัมน์ “ใช้(ล.)” คันเหรินเหอควรเป็น 70 ต่อเที่ยว

---

## 2026-05-22 (Session Summary #194 - แผน 23.05.26 + GPS 18:12)

### บริบทจากผู้ใช้
- อัปเดตแผน `23.05.26.txt` + GPS `…22.05.2026_18-12-44.xlsx`

### การตัดสินใจรอบนี้
- แก้ Remark ไม่ดึง Spare (เลิกแทน 8681→8684 ผิดวันนี้); NHL วันนี้ `วาฬ4ตู้2+KATOEN1` — KATOEN โชว์หลัง Closing
- ไม่มีเหรินเหอวันนี้; ไม่ใส่ `--add-fuel` (เติมคืนนี้ 0 ล.)

### สิ่งที่ทำแล้ว
- rebuild HTML/XLSX 23.05.26 — 7 คัน (KAO 8681, Conti×2, คลังวาฬ×2, KATOEN, Oatside)

### Action ถัดไป
- โอ Ctrl+F5 หน้าแผน; กรอกลิตรเติมจริงบน HTML

---

## 2026-05-22 (Session Summary #195 - สถานะกำหนดเอง + GPS fallback)

### บริบทจากผู้ใช้
- อยากเลือกสถานะ **เติมแล้ว / ต้องเติม** เองได้ (ไม่ยึดแค่ระบบ)
- GPS ไฟล์ล่าสุดบางคันไม่มี (รถหยุด/ลา) → ดึงจากไฟล์เก่าถัดไปใน `gps_inbox`

### การตัดสินใจรอบนี้
- คอลัมน์สถานะ: dropdown อัตโนมัติ | เติมแล้ว | ต้องเติม | ปกติ — จำใน localStorage ต่อไฟล์แผน
- `merge_fuel_gps_with_fallback`: คันขาดในไฟล์หลัก → ไล่ไฟล์ .xlsx เก่า (สูงสุด 12 ไฟล์); แสดง ↩ ชื่อไฟล์ + badge GPS เก่า

### สิ่งที่ทำแล้ว
- แก้ `build_lcb_fuel_dispatch_from_plan.py`; rebuild 23.05.26 — ตัวอย่าง 71-8682 ใช้ GPS จากไฟล์ 21.05.2026

### Action ถัดไป
- โอ Ctrl+F5; ทดสอบเปลี่ยนสถานะแถว; วางไฟล์ GPS เก่าหลายวันใน gps_inbox ถ้ามีรถหยุดหลายวัน

---

## 2026-05-23 (Session Summary #196 - PDF ปั๊มเต็กย้ง เฟส 1)

### บริบทจากผู้ใช้
- ปั๊มส่ง PDF รายงานเติมน้ำมันทุกเช้า · เพดานติดลบ ~4–5 หมื่น · ต้องการประเมินหนี้ปั๊มคู่กับแผน LCB

### สิ่งที่ทำแล้ว
- `parse_pump_credit_pdf.py` + `reports/pump_inbox/` + `pump_credit_latest.json`
- แถบ **บัญชีปั๊มเต็กย้ง** บนหน้า dispatch: ยอดปิด, หนี้, เหลือก่อนเพดาน, ต่ำสุดเดือน, เติมล่าสุดคันในแผน, โอนวงเงิน
- คาดการณ์หลังแผนเติมวันนี้ (อัปเดตตามช่องเติม + ราคา diesel)
- build อ่าน PDF ล่าสุดใน pump_inbox อัตโนมัติ (`--credit-limit 50000`)
