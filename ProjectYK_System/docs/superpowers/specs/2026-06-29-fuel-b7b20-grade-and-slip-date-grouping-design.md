...
**ทำ:**
1. สลิป (`_slip_body.html`): (ก) กลุ่ม DailyJob ที่ `work_date` เดียวกัน → โชว์วันที่ครั้งเดียว, แถวถัดมาในวันเดียวกันเว้นช่องวันที่ (หรือใส่ "↳"); (ข) โชว์ป้ายเกรด B7/B20 ท้ายช่องน้ำมันต่อบิล. ใช้ได้ทั้ง branch ปกติและ mixed.
...
- **โชว์ป้ายเกรด (ยืนยันแล้ว — โอ "เท่ห์ดี")**: ถ้า FuelTxn มี grade → ป้ายเล็ก "B7"/"B20" ท้ายช่องน้ำมัน ให้คนขับเห็นว่าเป็นคนละเกรด. **ต้องมี mapping job→grade** (ดูข้อ 5). ป้ายว่าง→ไม่โชว์. สไตล์ป้าย: เล็ก สีกลาง (เช่น `.tag-grade`) ไม่ชนกับป้าย หัก/ไม่หัก/บริษัท ที่มีอยู่.
