...
0. **ตรวจความพร้อมล่วงหน้า (ทำก่อนวันปิด 3-5 วัน): เช็ค"ชีทสด"ที่ทีมคีย์ ไม่ใช่แค่ไฟล์ local** —
   **LCB มีเครื่องมือสำเร็จรูปแล้ว (13ก.ค.):** `ProjectYK_System\app\.venv\Scripts\python.exe ProjectYK_System\tools\check_lcb_daily_keyed.py`
   (read-only — นับต่อวันทั้งรอบ: แถวเดลี่/แถวที่ลงค่าเที่ยวพขร. หา col จาก header เอง; อัปเดต `--tab` + CYCLE_* ในไฟล์เมื่อขึ้นรอบใหม่)
   วันไหน 0 ให้เช็คไขว้สดย่อย LCB (gsheet id ใน memory lcb-jul-preclose, วันที่ format "21 มิ.ย. 26") — เคส ก.ค.: เจอ 3 วันอาทิตย์ขาดทั้งที่มีงานรถจริง —
   ไฟล์ xlsx ใน `Salary/` เป็นสำเนา ณ วันหนึ่ง ของจริงที่ทีมคีย์อยู่คือ Google Sheet:
   - **LCB**: เล่มเดลี่แหลม sheet id `1Tm1i7kHGkiYtNwM-HQWEnQReg6VZmLzj7T1DjXr8zqg` แท็บรอบปัจจุบัน `Daily 16.MM.YY - 15.MM.YY` (หาคอลัมน์จาก header แถว 2 อย่าอิงตัวอักษรคอลัมน์ — layout เปลี่ยนตามปี) → ดูว่าคีย์จริงถึงวันไหน (เคส ก.ค.: เหลือ 5 วันปิดแต่คีย์ค้างที่ 23/6 — ต้องตามทีมทันที ไม่ใช่รอวันปิด)
   - **AYU**: ชีทจริง id `1F5eJlYs…` (ดูข้อ 1)
   - สดย่อยวังน้อย: แท็บสัปดาห์ล่าสุดต้องต่อเนื่องถึงปัจจุบัน (เคส ก.ค.: ขาดตั้งแต่ 21/6)
   เข้าถึงชีทด้วย service account ตามปกติ (gspread, `open_by_key` เท่านั้น) — ถ้าพบคีย์ค้าง แจ้งโอ+ทีมทันทีพร้อมวันที่ค้าง
1. **เดลี่ครบรอบ** — import จบแล้ว ไม่มีเที่ยวค้าง. **AYU รอบ 2026-07 import กลางรอบไปแล้ว 6ก.ค. (314 งาน) + โอเคาะกติกา: ข้อมูล AYU แก้ที่"ชีทจริง"เท่านั้น ห้ามแก้ใน grid** → ก่อนปิดรอบ 25ก.ค. ให้ re-import สุดท้าย: export ชีทจริง (id 1F5eJlYs…) เป็น xlsx → scp ทับ `YK_MVP/tools/ayu_gsheet.xlsx` บน server → รัน `_run_ayu_import.ps1` (dry-run ดูยอด) → apply พร้อม `--wipe-prior` (ลบ source=ayu_2026-07 แล้วเขียนใหม่ทั้งก้อน — ปลอดภัยเพราะไม่มีใครแก้ grid). **ถ้าพบว่ามีใครแก้ grid AYU ไปแล้ว: ห้าม wipe เด็ดขาด** (ของหาย — ตระกูล C4) — ท่าแก้: diff ชีทสด↔DB รายแถวหา field ที่ต่าง → ให้โอ/ทีมยืนยันรายแถวว่าค่าถูกคือฝั่งไหน → คีย์ค่าถูกกลับ**ชีทจริง** (ขออนุมัติ+ใส่ note ตามกฎ gsheet) แล้วค่อย wipe+reimport (หรือถ้าเป็น re-import ครั้งสุดท้ายก่อน finalize: wipe แล้ว re-apply ค่าในกริดซ้ำ — แต่ต้องตามคีย์ชีทให้ตรงภายหลัง). อนึ่ง importer **ไม่มีโหมด upsert**: no-wipe = add ซ้ำทั้งรอบ (dupe) ห้ามใช้เลี่ยง wipe. **ก่อน wipe ทุกครั้ง**: เช็คว่าแถวรอบนั้นยังไม่ถูกประทับเลขใบวางบิล (v52) หรือถูกอ้างจากระบบอื่น — wipe สร้าง id ใหม่หมด ของที่อ้าง id เดิมจะหลุด. ไซท์อื่น (LCB/BIGC) กติกาเดิม: import ครั้งเดียวตอนจบรอบ แล้วแก้ใน grid — ห้าม re-import ทับ.
