...
- **สดย่อยจากสลิปโอน**: มีท่อ LINE slip ingest + slip-config อยู่แล้ว → ต่อยอด: อ่านช่วยจำในสลิป + เดา คนขับ/ลาน/ตู้ จากข้อมูลแวดล้อม (เดลี่วันนั้น) เสนอให้คนยืนยัน — ไม่ auto ลงเงินเอง
...
- **อ่าน GPS ผ่าน username เว็บ vendor (ไม่ซื้อ API)**: ทำได้แบบ scraping แต่**เปราะ** (เว็บเปลี่ยน=พัง) + สุ่มเสี่ยงผิด ToS — ถ้าจะทำ ให้เป็น "ตัวเสริม" ไม่ใช่แกนระบบ; เริ่มจากถามผู้ให้บริการก่อนว่ามี export/รายงานอัตโนมัติไหม (หลายเจ้ามีส่งเมลรายวันฟรี → ใช้ email_ingest ที่มีอยู่รับได้เลย!)
