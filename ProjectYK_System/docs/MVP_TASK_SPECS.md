...
### A2 ✅ เสร็จ 4ก.ค. 09:2x — ใบเสร็จ+ใบหัก ณ ที่จ่าย + Doc Designer (โอเคาะ: "เอาตามมาตรฐานเลย + ให้ user ออกแบบปรับต่อได้")
**ของจริง:** `/kb-payout/receipt?invs=A,B` (ปุ่ม 🧾 ต่อใบ + ช่องพิมพ์หลายใบบนหัวหน้า KB) → หน้า A4 พิมพ์ได้ 2 แผ่น: **ใบเสร็จรับเงิน** (ยอด = KB เต็มของชุด + ตัวอักษรไทย + note โอนคืน 90%+บัญชีเจ้าของงาน) + **หนังสือรับรองหัก ณ ที่จ่าย 50 ทวิ** (3% ของ KB เต็ม, ประเภทเงินได้ ม.40(2), ช่องเลขบัตร/ที่อยู่ผู้ถูกหักเว้นไว้เติมมือ/แก้ใน designer); เลขเอกสาร `DocIssue` **idempotent** (ชุดเดิม = เลขเดิม ไม่เปลืองเลข); เคสจริงสเปค 1,713 → 51.39 → 1,541.70 อยู่ในเทสต์; ต้องติ๊กรับก่อนถึงออกใบได้ + ชุดต้องเจ้าของงานรายเดียว
**🎨 Doc Designer (`/admin/doc-designer` เมนูเครื่องมือ):** แก้ฟอร์มเองบน A4 จริง — ลากย้าย/มุมขวาล่างขยายหด/ดับเบิลคลิกแก้ข้อความ ({{placeholder}} ได้)/เพิ่มข้อความ-กรอบ-เส้น/A±/ตัวหนา/จัดแนว → เก็บ `DocTemplate` (v43) override default ในโค้ด (`services/doc_templates.py`), ปุ่ม ↺ กลับมาตรฐาน, ทุกการแก้เข้า AuditLog; **ฟอร์มใหม่ในอนาคต = เพิ่ม key ใน DEFAULT_TEMPLATES แล้วได้ designer ฟรี**
**รอบัญชีตรวจ:** ให้บัญชี/สรรพากรของโอดูฟอร์ม 50 ทวิ 1 รอบก่อนใช้ยื่นจริง (โครงตามมาตรฐาน แต่สำนักบัญชีแต่ละที่อาจอยากได้ช่องเพิ่ม — แก้เองได้ใน designer เลย)

สเปคเดิม (อ้างอิง): ขอโอส่งตัวอย่างก่อน — โอเคาะ 4ก.ค. ให้ใช้มาตรฐานได้เลย
...
### D1 🔶 เครื่องมือพร้อม 4ก.ค. 09:5x — เติมราคาเดลี่ (เหลืองานข้อมูลทีมไล่กด)
**เครื่องมือใหม่:** `⚡ /billing/fill-prices` (ปุ่มบน /billing + การ์ดหน้าแรกชี้มาที่นี่) — แถวงานราคาว่างของเดือน + **เรทที่ระบบจำ** (RateCard match ตาม ไซท์/ลูกค้า/เส้นทาง/ชนิดรถ ผ่าน `rate_find` เดิม) → คนกดรับต่อแถว (pattern B3: server หาเรทซ้ำเอง, มีราคาแล้ว = 409 ไม่ทับ, DailyJobAudit action `rate_apply`, บวก use_count); แถวไม่มีเรทจำ = ลิงก์ไป grid วันนั้นตรงๆ (คีย์แล้วระบบจำเป็นเรทให้รอบหน้าอัตโนมัติ); เทสต์ 3 ตัว tests/test_fill_prices.py
**เหลือ (งานข้อมูล ไม่ใช่โค้ด):** ทีม/โอ นั่งไล่กดรับ+เติม grid ให้ %priced ต่อไซท์ ≥95% (ตอนนี้ LCB 63/BigC 9)
...
**วัดจริงรอบแรก 4ก.ค. (window 12-15 มิ.ย. — ช่วงเดียวที่ archive ซ้อนเดลี่):** 340 รูป / any-match 91% / strong 57→80 หลังจูน **reverse-match เลข Job** (job_ref เดลี่โผล่ในข้อความรอบรูป = score 4 เช่น KLND "Job. KLND26-015737" ตรง job_ref 100%; doc_no DHL ตัด `"` นำ + `/xxx` ท้าย) — deploy แล้ว; mark กลุ่มไลน์ 38 กลุ่มลง server DB (19 customer อิงจาก status_code จริงในเดลี่ — แก้ได้ที่ /line/inbox); **บล็อคที่เหลือ:** (1) เดลี่ ก.ค. ยังไม่ import → วัดรอบเต็มไม่ได้ (วัดซ้ำ: `python measure_pod2.py 2026-07-01 2026-07-31` บน server) (2) NHL (กลุ่มรูปเยอะสุด 127 รูป) เลขงานอยู่ในตัวรูป ไม่มีใน text → ต้อง OCR (โยงงาน F4)
...
### F4 ✅ เสร็จ 4ก.ค. (เย็น) — น้ำมันจากไลน์เทียบระบบ **แบบไม่ใช้ OCR**
**ของจริง:** `/fuel/line-compare` + `services/fuel_line_compare.py` — ข้อค้นพบ: กลุ่มปั๊มโพสต์ **ข้อความแจ้งเติมมีโครงสร้าง** (ทะเบียน+เกรด+ลิตร เช่น "เติมดีเซล [20L]+B20 [80L]", "B20 (เต็มถัง)") → parse text ตรงๆ ปลดเงื่อนไขค่า OCR ที่รอโอเคาะ; จับคู่ FuelTxn ทะเบียนตรง+วัน±1+ลิตร±5 ต่อเกรด (เต็มถัง=ไม่เช็คลิตร, dedupe โพสต์ซ้ำ, ยอดติดลบไม่ร่วม); 4 กลุ่มผล: จับคู่/ไลน์แจ้งระบบไม่มี/ระบบมีไลน์ไม่แจ้ง/**รอ import ตามความสดของไซท์ตัวเอง** (gotcha วัดจริง: ใช้ max รวมธง LCB ผิด 175 รายการ — ไซท์ดูจาก Vehicle master); เมนู หน้างาน→⛽💬; เทสต์ 4 ตัว
**วัดจริง 12-25 มิ.ย.:** orders 403 / จับคู่ 53 / **ตกหล่นจริง 7 รายการ** (เช่น 71-0558 เต็มถัง 22 มิ.ย. ไม่มีในระบบ — รอทีมไล่เช็คตอน reconcile) / sys_only 44 (ส่วนใหญ่ขอบ 12 มิ.ย. ที่ archive เพิ่งเริ่ม); OCR รูปสลิปเหลือไว้เฉพาะถ้าอยากเทียบระดับยอดเงิน/ใบเสร็จ (ไม่จำเป็นสำหรับกันตกหล่น)

สเปคเดิม (อ้างอิง): — **⚠️ recon 4ก.ค.: สมมติฐานสเปคไม่ตรงของจริง**
...
### G2 ✅ เสร็จ 4ก.ค. — ย้ายรูปเก่าลง External HDD (v44)
**ของจริง:** ตาราง `MediaArchive` (v44 create_all — line_message_pk unique, disk_label, archive_path, sha256); `services/media_archive.py` (RETENTION_DAYS=730, เตือนดิสก์ <25%); การ์ดบน `/admin/server-health` โชว์รูปเก่าสุด/วันครบกำหนด/จำนวนครบกำหนด + ปุ่มย้ายต่อแผ่น 2 โหมด (ครบกำหนด >2ปี / เดือนเก่าสุดตอนดิสก์ตึง) วิ่งใน thread แบบเดียวปุ่ม Backup; แผ่นถูกจำด้วย `YK_ARCHIVE.txt` รากแผ่น (label EXT-01,02,… seq ใน AppSetting `media_archive_disk_seq`); ย้ายต่อไฟล์ copy→sha256 ตรง→จดแถว→ค่อยลบต้นทาง (hash ไม่ตรง = เก็บต้นทางไว้); `/line/media/{id}` fallback → เสิร์ฟจากแผ่น (หา label แม้อักษรไดรฟ์เปลี่ยน — ยึด YK_MEDIA ใน path) → แผ่นไม่เสียบ = ป้าย SVG "รูปอยู่แผ่น EXT-xx"; ZIP หลักฐาน (/billing/evidence) fallback เหมือนกัน; เทสต์ 4 ตัว tests/test_media_archive.py (100 ไฟล์ตามเกณฑ์ผ่าน+unplug+idempotent) — **เหลือ:** โอเสียบแผ่นจริงครั้งแรก (ปุ่มโผล่เอง)

สเปคเดิม (อ้างอิง): — โมเดล: Sonnet+ใหญ่รีวิว (~1 วัน เมื่อถึงเวลา)
