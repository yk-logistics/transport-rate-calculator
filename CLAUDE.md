...
| [`ProjectYK_System/docs/DATA_TOPOLOGY.md`](ProjectYK_System/docs/DATA_TOPOLOGY.md) | งานที่ต้องหา/ตรวจข้อมูลจริง (ข้อมูลจริงอยู่ไหน เชื่อได้แค่ไหน — server/mirror/dev) |
...
2. **เรื่องเล็ก ตัดสินใจเองแล้วจดข้อสมมติไว้** (ชื่อตัวแปร, ค่า default, ทางเลือกที่เทียบเท่ากัน) — อย่าถามโอ อย่าปิดท้ายด้วย "ต้องการให้…ไหมครับ?" สำหรับก้าวถัดไปที่ชัดอยู่แล้ว; หยุดถามเฉพาะ **เงินออกกระเป๋า / ลบ-ทับข้อมูล / ความกำกวมโดเมนที่เดาแล้วเงินเข้าผิดคน** — และ**อิสระ = วิธีทำงานที่ถูกขอ ไม่ใช่ขยายขอบเขต**: เจอของนอกคำขอที่อยากเก็บกวาด/ปรับปรุง (branch เก่า, โค้ดตาย, จัดระเบียบ) ให้**เสนอ อย่าทำ**
   **เช็คก่อนจบทุกเทิร์น (โอสั่งตรง 12 ก.ค. — Opus ชอบทำแล้วหยุดถาม):** ถ้าย่อหน้าสุดท้ายของตัวเองเป็น**คำถาม / ตัวเลือกให้เลือก / "แจ้งได้เลยครับ" / แผนที่ยังไม่ได้ทำ** ทั้งที่ไม่เข้า 3 เคสข้างบน = งานยังไม่จบ **ลบคำถามทิ้งแล้วกลับไปทำต่อจนจบ** — ติดปัญหาเทคนิคให้ลองเอง 2-3 เส้นทางก่อน (โอเป็น non-coder ถามไปงานแค่หยุดเฉยๆ); ถ้าตัดสินใจแทนได้ให้ตัดสินใจแล้ว**จดข้อสมมติไว้ในรายงานจบงาน** โอมาแก้ทีหลังได้เสมอ ถูกกว่างานค้างข้ามคืน
...
4. **อย่ารอใครสั่งให้หยิบเครื่องมือ** (Opus มักไม่หยิบเอง): เจอ gotcha/ตัดสินใจอะไร → เขียน `.claude/memory/` **ในเทิร์นนั้น**; งานอ่าน/สำรวจเยอะ → dispatch Qwen/Haiku ตาม delegation ladder; งานโค้ดที่มี test harness (แอปนี้มีเสมอ) → **เขียนเทสต์ก่อนโค้ด เห็นแดงก่อนเขียว** — "งานเล็ก/mechanical เกินกว่าจะ TDD" ไม่ใช่ข้อยกเว้น เพราะ harness พร้อมอยู่แล้วต้นทุนแทบศูนย์; ก่อนอ้างว่าเสร็จ → **เลือกระดับ verify อย่างตั้งใจแล้วบอกไว้ในรายงาน** (เทสต์พอ? หรือต้อง boot แอป/ยิง endpoint/เปิดหน้าจริง?) — ห้ามให้ "เทสต์ผ่าน = เสร็จ" เกิดขึ้นโดยไม่ได้ชั่งใจ
